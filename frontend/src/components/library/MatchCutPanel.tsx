...
export function MatchCutPanel({ asset, shots, initialShot }: { asset: AssetDetail; shots: Shot[]; initialShot?: Shot }) {
  ...
  const searchKey = JSON.stringify([shotNumber, frameSeconds, targetIds, weights, region, indexes.map(i => [i.job_id, i.revision])])
  ...
  async function find() {
    if (!shot || !sourceIndex) return
    request.current?.abort()
    const controller = new AbortController(); request.current = controller
    setWorking(true); setError(''); setNotice('')
    try {
      const data = await visualRequest<VisualResults>(`${asset.job_id}/search`, {
        shot_number: shot.shot_number, seconds: frameSeconds, revision: sourceIndex.revision,
        target_ids: targetIds, ...weights, region,
      }, controller.signal)
      if (controller.signal.aborted) return
      setResult(data); setResultKey(searchKey); setSelected(null)
  ...
  function exportPair() {
    ...
    const data = { version: 1, type: 'visual_match_cut', timing: 'media_file_seconds', timestamp_accuracy: 'approximate; verify in editor',
      ...
      discovery: { sampled_outgoing_seconds: frameSeconds, sampled_incoming_seconds: selected.seconds,
        scores: selected.scores, score_basis: 'local_visual_measurements_not_confidence', weights, source_region: region } }
  ...
  return <section className="mc-panel" aria-label="Visual match-cut discovery">
    <div className="mc-heading"><div><h2>Match cuts</h2><p>Match shape, composition, and color across your footage.</p></div><button className="lw-button lw-button-primary mc-find" onClick={find} disabled={working || !available || !targetIds.length || !sourceIndex || seconds !== frameSeconds || !Object.values(weights).some(Boolean)}>{working ? 'Finding matches…' : 'Find match cut'}</button></div>
    ...
    <div className="mc-index"><div role="status">{targets.some(i => i.status === 'indexing') ? 'Indexing' : 'Visual index'} · {available} / {targets.reduce((n, i) => n + i.total, 0)} frames{targets.some(i => i.failed) ? ' · Some frames failed; resume to retry.' : ''}</div>
      ...
      {targets.some(i => i.status === 'indexing') && <button className="lw-text-button" onClick={async () => { try { await Promise.all(targetIds.map(id => visualRequest(`${id}/index/cancel`, {}))); setNotice('Index paused. Saved frames remain searchable.') } catch (err) { setError(errorMessage(err)) } }}>Pause</button>}
      <small>Local index · no API calls · five frames per shot</small>
    </div>
    {[...indexErrors, ...indexes.flatMap(i => i.error ? [i.error] : []), ...(error ? [error] : [])].map((message, i) => <p className="lw-inline-error" role="alert" key={i}>{message}</p>)}
    {notice && <p role="status">{notice}</p>}
    <div className="mc-pair">
      <div><div className="mc-frame-heading"><strong>A · Outgoing</strong><span>{time(frameSeconds)}</span></div>
        <FrameRegion url={sourceIndex ? visualFrame(asset.job_id, shot.shot_number, frameSeconds, sourceIndex.revision) : undefined} region={region} box={!stale ? selected?.source_box : null} onRegion={setRegion} />
        ...
        <div className="mc-frame-heading"><small>Drag over a shape to focus. Color compares the whole frame.</small><button className="lw-text-button" onClick={() => setRegion([.25, .25, .5, .5])}>Center region</button>{region && <button className="lw-text-button" onClick={() => setRegion(null)}>Whole frame</button>}</div>
    ...
    {result && !stale && <><div className="mc-results-title"><h3>{result.matches.length} candidate{result.matches.length === 1 ? '' : 's'}</h3><small>Relative visual similarity · not confidence. Motion is not scored yet.</small></div>
      {!result.matches.length && <p>No candidates yet. Index more shots, change the frame, or widen the region.</p>}
      <div className="mc-results">{result.matches.map(match => <button key={`${match.job_id}-${match.shot_number}`} className={`mc-card ${selected === match ? 'is-selected' : ''}`} aria-pressed={selected === match} onClick={() => choose(match)}><div style={{ aspectRatio: match.aspect_ratio }}><img src={match.frame_url} loading="lazy" alt={`Candidate shot ${match.shot_number} at ${time(match.seconds)}`} /><Box box={match.target_box} /></div><strong>Shot {String(match.shot_number).padStart(2, '0')} <time>{time(match.seconds)}</time></strong><span>{match.title}</span><p>{match.description}</p><div className="mc-scores">{AXES.map(axis => <span key={axis}>{axis} <b>{Math.round(match.scores[axis] * 100)}</b></span>)}</div></button>)}</div>
    </>}
...
function FrameRegion({ url, region, box, onRegion }: { url?: string; region?: number[] | null; box?: number[] | null; onRegion?: (region: number[] | null) => void }) {
  ...
  return <div className={`mc-frame ${onRegion ? 'can-select' : ''}`} style={{ aspectRatio: aspect }} aria-label={onRegion ? 'Outgoing frame. Drag to select a region.' : 'Incoming frame'} onPointerDown={event => { if (!onRegion || loaded !== url || event.button !== 0) return; anchor.current = point(event); event.currentTarget.setPointerCapture(event.pointerId) }} onPointerMove={event => { if (anchor.current) setDrag(rectangle(point(event))) }} onPointerUp={event => { if (!anchor.current) return; const r = rectangle(point(event)); anchor.current = null; setDrag(null); onRegion?.(r[2] >= .02 && r[3] >= .02 ? r : null) }} onPointerCancel={() => { anchor.current = null; setDrag(null) }}>
