...
export function MatchCutPanel({ asset, shots, initialShot }: { asset: AssetDetail; shots: Shot[]; initialShot?: Shot }) {
  ...
  return <section className="mc-panel" aria-label="Visual match-cut discovery">
    ...
    {(weights.shape > 0 || readingShape) && <div className="mc-composition-status" role="status">{readingShape ? 'Identifying shapes' : 'Shape cache'} · {targets.reduce((n, i) => n + (i.shape?.indexed || 0), 0)} / {targets.reduce((n, i) => n + (i.shape?.total || 0), 0)} frames{readingShape && ' · Matches update as silhouettes arrive.'}</div>}
    ...
    {result && !stale && <><div className="mc-results-title"><h3>{result.matches.length} candidate{result.matches.length === 1 ? '' : 's'}</h3><small>Relative visual similarity · not confidence. Motion is not scored yet.</small></div>
      {!result.matches.length && <p>{readingShape || readingComposition ? 'Reading frames. Compatible candidates will appear here.' : weights.shape ? 'No close silhouettes found. Try another shape or search more projects.' : weights.composition ? 'No compatible framing found in the analyzed frames. Try another source frame or search more projects.' : 'No candidates yet. Index more shots, change the frame, or widen the region.'}</p>}
