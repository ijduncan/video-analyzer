  ...
  return <aside className="lw-inspector" aria-label="Footage details">
    ...
    <div ref={body} className="lw-inspector-body" role="tabpanel" id={`asset-panel-${tab}`} aria-labelledby={`asset-tab-${tab}`}>
      ...
      {tab === 'analysis' && <>
        ...
        <button className="lw-button lw-button-primary lw-full" disabled={busy || removing || asset.status === 'deleting'} onClick={analyze}>{busy ? 'Analyzing…' : !capabilities?.google_configured && !apiKey ? 'Connect Gemini' : shots.length ? 'Analyze again' : 'Analyze'}</button>
