...
4. For **Shape**, use **Identify shapes**, then select a visible object or part from the **Shape** list, click its outline, or draw around it. The source silhouette is fixed before any candidate is scored. **Match position and size too** is enabled by default and requires similar screen placement and occupancy; turn it off to search for form alone. A drawn region chooses among identified objects; it does not perform an arbitrary crop segmentation. Composition evaluates the whole frame; color compares whole-frame palettes.
